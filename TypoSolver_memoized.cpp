...
private:
    ...
    struct Choice {
        ...
        int advanceI = 0;
        int advanceJ = 0;
        int emitTransposeCount = 0; // 0 for non-transpose, 1 for normal transpose, 2 for two consecutive adjacent transpositions
    };
...
private:
    ...
    int solve(int i, int j) {
        ...
        // Two consecutive allowed adjacent transpositions handled as one memo transition,
        // but reconstructed as two standard Transpose lines.
        if (i + 2 < n && j + 2 < m &&
            target[i + 1] == typo[j] &&
            target[i + 2] == typo[j + 1] &&
            target[i]     == typo[j + 2]) {
